 ...
 */
export function resolveModelDisplayName(modelId: string, channels: import('@myyoda/shared').Channel[]): string {
 ...
 */
export function resolveModelProvider(modelId: string, channels: import('@myyoda/shared').Channel[]): ProviderType | undefined {
  for (const channel of channels) {
    for (const model of channel.models) {
      if (model.id === modelId) {
